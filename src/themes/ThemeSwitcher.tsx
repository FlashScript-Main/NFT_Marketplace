...
const ThemeSwitcher = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
        <>
            <h5 className={`mb-2 | text-nftCustom-background text-base md:text-lg font-medium |  | ${language("isEnglish") === "true" ? "" : `${iranSans} text-end mb-3`}`}>
                ...
                {customThemes.map((themeOption) => (
                    <button 
                        ...
                        className={`
                            ...
                            ${themeOption.value === "nftDefault" && "max-md:col-span-2 xl:col-span-2"}
                            ${themeOption.value === "light" && "xl:col-span-2"} 
                            ${themeOption.value === "dark" && "xl:col-span-2"} 
