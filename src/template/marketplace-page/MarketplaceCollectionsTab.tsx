...
import { marketplaceCollectionsTabInfo } from "@/constant/nft-database";
import { useTranslations } from "next-intl";
import Image from "next/image";
import Link from "next/link";

const MarketplaceCollectionsTab = ({ locale }: { locale: string }) => {
    ...
    return (
        <div className={`mt-[3.5rem] |  |  | `}>
            <DivToScroll className={` |  | grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-[1.875rem] | `}>
                {marketplaceCollectionsTabInfo.map((collection, index) => (
                    <MotionDiv
                        key={collection.id}
                        ...
                        >
                            ...
                            <div className={`py-7 px-7 xl:py-9 xl:px-9 | bg-nftCustom-background |  | `}>
                                <MotionH6 
                                    ...
                                    whileInView={{ x: "0%", opacity: 1 }}
                                    viewport={{ once: true, margin: "-50px" }}
                                    transition={{ staggerChildren: 0.02, delay: 0.25 * (index + 0.05), duration: 0.5 }}
...
export default MarketplaceCollectionsTab